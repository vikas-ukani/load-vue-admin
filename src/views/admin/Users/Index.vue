<template >
  <div class="animated fadeIn">
    <div class="card card-body pl-0 pr-0 pt-0">
      <div v-for="(user, index) in users" :key="index" class="background-color-grey m-3 p-3 mr-3">
        <div class="row">
          ...
          <div class="col-md-10">
            ...
            <div class="row">
              <div class="col-md-12">
                <label for="email" class="font-sm">{{ user.email }}</label>
                <label for="date_of_birth" class="font-sm" v-if="user.date_of_birth">
                  <b class="pl-1 pr-1">|</b>
                  Birth day {{ user.date_of_birth }}
                </label>
                <br />
                <label for="country_detail" class="font-sm">
                  Member since
                  <b>NULL</b>.
                </label>
                <label
                  for="country_detail.name"
                  class="font-sm"
                  v-if=" user.country_detail &&  user.country_detail.name"
                >
                  <b class="pl-2 pr-2">|</b>
                  Country:
                  {{ user.country_detail.name || '-' }}
                </label>
                <label
                  for="account_detail.name"
                  class="font-sm"
                  v-if=" user.account_detail &&  user.account_detail.name"
                >
                  <b class="pl-2 pr-2">|</b>
                  Membership:
    ...
    <div v-if="!users" class="no-data-found mt-5 col-md-12 display-4 text-center text-danger">
