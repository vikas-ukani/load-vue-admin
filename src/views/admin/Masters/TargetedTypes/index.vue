...
export default {
  ...
  methods: {
    ...
    /** get details by id */
    async getDetails(id) {
      let res = await Services.call(ApiCollections.get_training_type).getOne(
        id
      );
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(ApiCollections.delete_training_type).delete(
        list.id
      );
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_training_type);
        ...
        /** create data */
        let res = await Services.call(ApiCollections.create_training_type).post(
          this.detail
        );
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      let res = await Services.call(ApiCollections.training_type_listing).post(
        request
      );
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.training_type_status_change
      ).post(request);
