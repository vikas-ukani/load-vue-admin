...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.training_intensity_delete_multiple
              ).deleteMany(request);
    ...
    async getDetails(id) {
      let res = await Services.call(
        ApiCollections.get_training_intensity
      ).getOne(id);
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(
        ApiCollections.delete_training_intensity
      ).delete(list.id);
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_training_intensity);
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_training_intensity
        ).post(this.detail);
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.training_intensity_status_change
      ).post(request);
