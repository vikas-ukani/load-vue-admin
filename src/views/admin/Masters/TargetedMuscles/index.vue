...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.targeted_muscles_delete_multiple
              ).deleteMany(request);
    ...
    /** get details by id */
    async getDetails(id) {
      let res = await Services.call(ApiCollections.get_targeted_muscles).getOne(
        id
      );
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(
        ApiCollections.delete_targeted_muscles
      ).delete(list.id);
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_targeted_muscles);
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_targeted_muscles
        ).post(this.detail);
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      let res = await Services.call(
        ApiCollections.targeted_muscles_listing
      ).post(request);
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.targeted_muscles_status_change
      ).post(request);
