        ...
        >
          <thead>
            <tr>
              ...
              <th class="text-capitalize">Targatd HR.</th>
          ...
          <tbody v-if="lists &&  lists.length">
            <tr v-for="(list, index) in lists" :key="index">
              ...
              <td>
                <small class="font-bold">{{ (list.target_hr )|| '-' }}</small>
              </td>
...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.training_goals_delete_multiple
              ).deleteMany(request);
    ...
    /** get details by id */
    async getDetails(id) {
      let res = await Services.call(ApiCollections.get_training_goals).getOne(
        id
      );
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(
        ApiCollections.delete_training_goals
      ).delete(list.id);
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_training_goals);
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_training_goals
        ).post(this.detail);
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      let res = await Services.call(ApiCollections.training_goals_listing).post(
        request
      );
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.training_goals_status_change
      ).post(request);
