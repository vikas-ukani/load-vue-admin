        ...
        <table class="table table-hover  table-responsive-sm col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <thead>
            <tr>
              ...
              <th class="text-capitalize">Trial days</th>
          ...
          <tbody v-if="lists &&  lists.length">
            <tr v-for="(list, index) in lists" :key="index">
              ...
              <td>
                <small class="font-bold">{{ (list.free_trial_days )|| '-' }}</small>
              </td>
              <td>
      ...
      <form method="post" novalidate name="addEditForm">
        <div class>
          ...
          <div class="input-group mb-3">
            ...
            <div class="input-group col-md-12">
              <b-form-select v-model="detail.code" readonly>
                <option :value="null">Please account type</option>
                <option
          ...
          <div class="input-group mb-3">
            <label for="free_trial_days" class="text-capitalize ml-3">
              Trial days
              <small
                :class="!detail.free_trial_days || errors.has('free_trial_days') ? 'text-danger' : 'text-success' "
              >*</small>
            </label>
            <div class="input-group col-md-12">
              <input
                ...
                id="free_trial_days"
                name="trial days"
                v-model="detail.free_trial_days"
                required="required"
...
export default {
  ...
  mounted() {
    _.each(Constants.Accounts, (key, val) => {
      this.accountList.push({ name: key, code: val });
    });
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.accounts_delete_multiple
              ).deleteMany(request);
    ...
    /** get details by id */
    async getDetails(id) {
      let res = await Services.call(ApiCollections.get_accounts).getOne(id);
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(ApiCollections.delete_accounts).delete(
        list.id
      );
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_accounts);
        ...
        /** create data */
        let res = await Services.call(ApiCollections.create_accounts).post(
          this.detail
        );
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      /** start progress here */
      this.$Progress.start();
      let res = await Services.call(ApiCollections.accounts_listing).post(
        request
      );
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(ApiCollections.accounts_status_change).post(
        request
      );
