...
export default {
  ...
  methods: {
    ...
    /** get details by id */
    async getDetails(id) {
      let res = await Services.call(ApiCollections.get_action_force).getOne(id);
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(ApiCollections.delete_action_force).delete(
        list.id
      );
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_action_force);
        ...
        /** create data */
        let res = await Services.call(ApiCollections.create_action_force).post(
          this.detail
        );
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      let res = await Services.call(ApiCollections.action_force_listing).post(
        request
      );
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.action_force_status_change
      ).post(request);
