...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.delete_cancellation_policy
              ).deleteMany(request);
    ...
    async getDetails(id) {
      let res = await Services.call(
        ApiCollections.get_cancellation_policy
      ).getOne(id);
    ...
    handleFileUpload(e) {
      this.detail.icon_path = e.target.files[0];
    },
    async submitData() {
      this.$Progress.start();
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(
          ApiCollections.update_cancellation_policy
        );
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_cancellation_policy
        ).post(this.detail);
        /** set data */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      let res = await Services.call(
        ApiCollections.cancellation_policy_listing
      ).post(request);
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.cancellation_policy_status_change
      ).post(request);
