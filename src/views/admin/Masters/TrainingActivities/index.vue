<template >
  <div class="animated fadeIn">
        ...
        >
          ...
          <tbody v-if="lists && lists.length">
            <tr v-for="(list, index) in lists" :key="index">
              ...
              <td>
                <!-- baseURL + list.icon_path -->
                <!-- v-bind:src="getImageLoaded(list.icon_path)" -->
                <img
                  v-bind:src="baseURL + list.icon_path"
    ...
    <b-modal
      ...
      @ok="submitData"
    >
      <form method="post" enctype="multipart/form-data" novalidate name="addEditForm">
        <div class="col-md-12">
          ...
          <div class="input-group">
            <!-- <pre>{{ baseURL + detail.icon_path }}</pre> -->
            <!-- baseURL + -->
            <div class="input-group col-md-12">
              <img
                v-if="detail.icon_path && detail.id"
                v-bind:src="detail.icon_path"
                class="img-thumbnail"
                width="120"
                height="150"
              />
...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.training_activities_delete_multiple
              ).deleteMany(request);
    ...
    async getDetails(id) {
      let res = await Services.call(
        ApiCollections.get_training_activities
      ).getOne(id);
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(
        ApiCollections.delete_training_activities
      ).delete(list.id);
      if (res && res.success && res.success == true) {
    ...
    handleFileUpload(e) {
      ...
      if (input.files && input.files[0]) {
        ...
        reader.onload = e => {
          this.detail.icon_path = e.target.result;
    ...
    async submitData() {
      ...
      // if (this.detail.icon_path) {
      //   formData.append("icon_path", this.detail.icon_path);
      // }
      if (this.selectedFile) {
        formData.append("icon_path", this.selectedFile);
      }
      // if (this.detail.id) {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(
          ApiCollections.update_training_activities
        );
        apiObject.url += this.detail.id;

        console.log("updated  Data", this.detail, formData);
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_training_activities
        ).post(formData);
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.training_activities_status_change
      ).post(request);
