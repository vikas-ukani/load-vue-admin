        ...
        <table class="table table-hover table-responsive-sm col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <thead>
            <tr>
              ...
              <!-- <th class="text-capitalize">Estimated weight</th>
              <th class="text-capitalize">Actual weight</th>-->
          ...
          <tbody v-if="lists &&  lists.length">
            <tr v-for="(list, index) in lists" :key="index">
              ...
              <td>
                <small class="font-bold">{{ (list.weight )|| '-' }}</small>
              </td>
              <!-- <td>
                <small class="font-bold">{{ (list.estimated_weight )|| '-' }}</small>
              </td>
              <td>
                <small class="font-bold">{{ (list.actual_weight )|| '-' }}</small>
              </td>-->
    ...
    >
      <form method="post" novalidate name="addEditForm">
        <div class="col-md-12">
          ...
          <div class="input-group mb-3">
            <label for="weight" class="text-capitalize ml-3">
              Weight
              <small
                :class="!detail.weight || errors.has('weight') ? 'text-danger' : 'text-success' "
              >*</small>
            </label>
            <div class="input-group col-md-12">
              <input
            ...
            <small v-if="errors.has('weight')" class="text-danger mt-1">{{ errors.first('weight') }}</small>
          </div>
...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.repetition_delete_multiple
              ).deleteMany(request);
    ...
    /** get details by id */
    async getDetails(id) {
      let res = await Services.call(ApiCollections.get_repetition_max).getOne(
        id
      );
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(
        ApiCollections.delete_repetition_max
      ).delete(list.id);
      if (res && res.success && res.success == true) {
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(ApiCollections.update_repetition_max);
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_repetition_max
        ).post(this.detail);
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      this.clearAllSelecte(); /** make common request */
      var request = {
        page: this.page,
        limit: this.limit
      };
      ...
      let res = await Services.call(ApiCollections.repetition_max_listing).post(
        request
      );
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.repetition_max_status_change
      ).post(request);
