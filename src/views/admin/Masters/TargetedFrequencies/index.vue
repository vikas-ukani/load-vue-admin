        ...
        <table class="table table-hover table-responsive-sm col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <thead>
            <tr>
              ...
              <th class="text-capitalize">Max days</th>
          ...
          <tbody v-if="lists &&  lists.length">
            <tr v-for="(list, index) in lists" :key="index">
              ...
              <td>
                <small class="font-bold">{{ list.title || '-' }}</small>
              </td>
              <td>
                <small class="font-bold">{{ (list.code )|| '-' }}</small>
              </td>
              <td>
                <small class="font-bold">{{ (list.max_days )|| '-' }}</small>
              </td>
              ...
              <td>
                <a
                  v-b-tooltip.hover.left="'Edit ' + list.title"
                ...
                <a
                  v-b-tooltip.hover.left="'Delete ' + list.title"
      ...
      <form method="post" novalidate name="addEditForm">
        <div class="col-md-12">
          <div class="input-group mb-3">
            <label for="title" class="text-capitalize ml-3">
              name
              <small
                :class="!detail.title || errors.has('title') ? 'text-danger' : 'text-success' "
              >*</small>
            </label>
            ...
            <small v-if="errors.has('title')" class="text-danger mt-1">{{ errors.first('title') }}</small>
          </div>
          ...
          <div class="input-group mb-3">
            <label for="max_days" class="text-capitalize ml-3">
              Max days
              <small
                :class="!detail.max_days || errors.has('max_days') ? 'text-danger' : 'text-success' "
              >*</small>
            </label>
            <div class="input-group col-md-12">
              <input
                ...
                id="max_days"
                name="max days"
                v-model="detail.max_days"
                required="required"
                placeholder="Enter free trial days"
                class="form-control"
                aria-required="true"
                aria-invalid="true"
                v-validate="'required|max_value:30'"
                :class="{ 'is-invalid': errors.has('trial days') }"
              />
              <!-- readonly -->
            </div>
            <small
              v-if="errors.has('trial days')"
              class="text-danger mt-1"
            >{{ errors.first('max days') }}</small>
          </div>
          <div class="input-group mb-3">
...
export default {
  ...
  methods: {
          ...
          if (result.value) {
            if (this.selectedIds && this.selectedIds.length) {
              let res = await Services.call(
                ApiCollections.targeted_frequencies_delete_multiple
              ).deleteMany(request);
    ...
    async getDetails(id) {
      let res = await Services.call(
        ApiCollections.get_targeted_frequencies
      ).getOne(id);
    ...
    deleteConfirmation(list) {
      ...
      this.$swal
        .fire({
          title: "Are you sure?",
          text: "You want to delete " + list.title + " record!",
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: "#3085d6",
          cancelButtonColor: "#d33",
    ...
    async deleteRecord(list) {
      ...
      let res = await Services.call(
        ApiCollections.delete_targeted_frequencies
      ).delete(list.id);
      if (res && res.success && res.success == true) {
        ...
        this.$swal.fire(
          "Deleted!",
          list.title + " record has been deleted.",
          "success"
        );
      }
    },
    ...
    async submitData() {
      ...
      if (this.detail && this.detail.id) {
        var apiObject = this.$_.clone(
          ApiCollections.update_targeted_frequencies
        );
        ...
        /** create data */
        let res = await Services.call(
          ApiCollections.create_targeted_frequencies
        ).post(this.detail);
        /** set data  */
        if (res && res.success && res.success == true) {
    ...
    async getAllList(search = "") {
      ...
      let res = await Services.call(
        ApiCollections.targeted_frequencies_listing
      ).post(request);
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.targeted_frequencies_status_change
      ).post(request);
